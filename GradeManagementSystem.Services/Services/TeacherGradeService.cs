...
namespace GradeManagementSystem.Services.Services
{
    public class TeacherGradeService : ITeacherGradeService
    {
        ...
        public async Task<object> SubmitGradeAsync(TeacherSubmitGradeDto dto )//, int teacherId)
        {
            ...
            if (!Enum.IsDefined(typeof(GradeType), dto.GradeType))
                throw new Exception("Invalid grade type");
            ...
            if (assignment.SubjectID == null)
                throw new Exception("Assignment has no subject");

            var academicYearId = await gradeDbContext.AcademicYears
                .Where(x => x.IsActive)
